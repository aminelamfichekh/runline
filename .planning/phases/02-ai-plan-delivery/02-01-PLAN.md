...
must_haves:
  truths:
    - "Initial plan generates automatically after subscription payment (first Monday after subscription as start date)"
    - "Generated plan contains complete weekly schedule with daily workouts in French"
    - "Monthly regeneration runs automatically every first Monday"
    - "Failed plan generation retries automatically (3 attempts)"
    - "User receives push notification when plan generation completes"
  artifacts:
  ...
  key_links:
    ...
    - from: "GeneratePlanJob.php"
      ...
      via: "notifyPlanGenerated after successful generation"
      pattern: "notifyPlanGenerated"
---
...
<task type="auto">
  <name>Task 1: Upgrade GeneratePlanJob to use OpenAI structured outputs and send notification</name>
  <files>
    emrun-backend/app/Jobs/GeneratePlanJob.php
    emrun-backend/app/Services/PlanGeneratorService.php
  </files>
...
6. Add NotificationService call after successful plan generation:
```php
use App\Services\NotificationService;

// In the handle() method, after plan is saved with status 'completed':
$notificationService = app(NotificationService::class);
$notificationService->notifyPlanGenerated($this->plan->user);
```

This ensures the user receives a push notification when their plan is ready.
...
Verify notification is called:
`grep -n "notifyPlanGenerated" emrun-backend/app/Jobs/GeneratePlanJob.php`
  </verify>
  <done>
...
NotificationService::notifyPlanGenerated() is called after successful plan generation.
  </done>
</task>
...
NOTE: In this app, "signup" means subscription payment. Users create an account, complete questionnaire, then subscribe. The initial plan starts on the first Monday AFTER the subscription payment date (which is the "signup" moment that triggers plan generation).
...
// Plan starts on first Monday after subscription (today)
...
if ($user->profile && $user->profile->questionnaire_completed) {
    ...
    if (!$existingPlan) {
        ...
        Log::info('Initial plan generation triggered for new subscriber', [
            'user_id' => $user->id,
            'plan_start_date' => now()->next('Monday')->toDateString(),
        ]);
    }
}
```

4. Verify PlanGeneratorService::generateInitialPlan() calculates start_date as first Monday after today:
```php
// In PlanGeneratorService::generateInitialPlan()
...
Verify start date calculation in PlanGeneratorService:
`grep -n "next.*Monday\|startOfWeek" emrun-backend/app/Services/PlanGeneratorService.php`
...
Plan start_date is first Monday after subscription payment.
...
3. GeneratePlanJob sends notification: grep for 'notifyPlanGenerated' in GeneratePlanJob.php
4. Webhook triggers plan: grep for 'generateInitialPlan' in WebhookController.php
5. Start date is first Monday: grep for 'next.*Monday' in PlanGeneratorService.php
6. Cron command exists: `php artisan plans:generate-monthly --help` shows command
7. No duplicate plan logic: grep for 'already exists' or existing plan check in both webhook and cron
</verification>

<success_criteria>
- GeneratePlanJob uses gpt-4o-2024-08-06 with json_schema response_format
- GeneratePlanJob calls NotificationService::notifyPlanGenerated() after successful generation
- WebhookController triggers generateInitialPlan on subscription.created
- Plan start_date is calculated as first Monday after subscription payment

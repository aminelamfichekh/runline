...
export default function Step9Screen() {
  ...
  const handleContinue = () => {
    router.push('/(questionnaire)/step10');
  };
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.content}>
          ...
          <Text style={styles.optional}>({t('onboarding.step9.optional')})</Text>
          <Text style={styles.subtitle}>{t('onboarding.step9.subtitle')}</Text>

          <TextInputField
            label=""
            value={watch('equipement')}
            onChangeText={(text) => setValue('equipement', text)}
            placeholder={t('onboarding.step9.placeholder')}
            multiline
            numberOfLines={5}
            optional
          />
